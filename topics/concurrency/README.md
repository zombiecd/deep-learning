...
> 取材于一次 Go 底层原理面试复盘。但复盘材料是参考不是源——里头有「结论对、机理错」的地方（已验证勘误见下），本主题逐条回一手资料 / 反汇编 / benchmark 重讲，不靠"听起来对"。
...
材料里 3 主问 + 5 延伸 + 4 手写，全是这五面墙的推论。复习时顺着墙看，不要背孤卡。

| 面试题（材料） | 主要落在 | 重讲时纠正 / 钻深的点 |
|---|---|---|
...
| 主3 原子为何轻 + 高竞争 | B · C | **`atomic.Add` 是单条 `LOCK XADD`，不是 CAS 自旋**（材料勘误）；高竞争崩在 cache 乒乓不在自旋 |
...
- **`atomic.AddInt64` 不是 CAS 自旋循环**。amd64 上是单条 `LOCK XADDQ`；arm64 上 Go 按 CPU 特性做**运行时分支**——支持 LSE 的芯片（如 Apple M 系列）走单条 `LDADDAL`，老 ARM 才回退到 `LDAXR/STLXR` 的 LL/SC 循环。"它是 CAS 自旋"是把抽象当实现的类别错误：具体指令随 ISA、甚至随 CPU 代次变。由此材料"高竞争原子操作崩在自旋"的归因也错了——真正瓶颈是 C 题的 cache line 乒乓。（一手反汇编验证）
...
| B 原子性硅层 | `02-atomicity-hardware.md` | ⏳ 待开 |
| C 核越多越慢 | `03-cache-coherence.md` | ⏳ 待开 |
| D 抢不到锁之后 | `04-blocking-scheduling.md` | ⏳ 待开 |
